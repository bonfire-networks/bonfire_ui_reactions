  ...
  mounted() {
    ...
    this.emojiClickHandler = async (event) => {
      ...
      try {
        ...
        if (database) {
          const unicodeOrName = event.detail.unicode || 
            (event.detail.emoji && (event.detail.emoji.name || event.detail.emoji.shortcodes && event.detail.emoji.shortcodes[0]));
      ...
      this.pushEventTo(this.el, "Bonfire.Social.Likes:add_reaction", {
        ...
        id: objectId,
        label: event.detail.emoji && event.detail.emoji.annotation
      });
